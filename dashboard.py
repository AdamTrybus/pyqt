...
class DashboardWidget(QWidget):
    ...
    def set_events(self, events):
        ...
        for row, event in enumerate(events):
            ...
            self.event_table.setCellWidget(row, 2, edit_button) # Dodanie przycisku do wiersza
    ...
    def edit_event_dialog(self, event):
        ...
        date_edit = QDateEdit(QDate.fromString(event['date'], "yyyy-MM-dd"), dialog)
        ...
        save_button.clicked.connect(lambda: self.handle_save_button_click(dialog, event, title_edit.text(), date_edit.date().toString("yyyy-MM-dd"), time_edit.time().toString("hh:mm")))

        delete_button.clicked.connect(lambda: self.handle_delete_button_click(dialog, event))
    ...
    def add_event(self):
        ...
        # Przypisanie funkcji do przycisków
        add_button.clicked.connect(lambda: self.handle_add_button_click(dialog, title_edit.text(), date_edit.date().toString("yyyy-MM-dd"), time_edit.time().toString("hh:mm")))
